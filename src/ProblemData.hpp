...
// seed for random permutation generation (any fixed value, you can change if
...
public:
  ...
  void SetCoordinates(const vector<Coordinate> &coordinates) {
    coordinates_ = coordinates;
  }
  void SetDemands(const vector<int> &demands) { demands_ = demands; }
  void SetPermutation(const vector<int> &permutation) {
    permutation_ = permutation;
  }
  void SetEdgeWeights(const vector<vector<double>> &edge_weights) {
...
private:
  ...
  int num_groups_; // number of groups/vehicles (loaded from .lcvrp file now
                   // instead of being set in entry file)

  vector<Coordinate> coordinates_; // node coordinates (indexed from 0, but node
                                   // 1 is at index 0)
  vector<int>
      demands_; // customer demands (indexed from 0, but node 1 is at index 0)
  vector<int>
      permutation_; // fixed permutation for LcVRP (customer indexes 1..n-1)
