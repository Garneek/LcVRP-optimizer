...
ProblemData::ProblemData()
    : dimension_(0), capacity_(0), distance_(0.0),
      has_distance_constraint_(false), depot_(1), num_groups_(0) {}
...
double ProblemData::CalculateDistance(int i, int j) const {
  ...
  if (edge_weight_type_ == "EUC_2D") {
    ...
    if (edge_weights_.empty() || i >= (int)edge_weights_.size() ||
        j >= (int)edge_weights_[i].size()) {
      return WRONG_VAL;
    }
    if (i > j) {
